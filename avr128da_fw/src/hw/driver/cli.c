...
{
  cliPrintf_P(PSTR("\n\r"));
  cliPrintf_P(PSTR(CLI_PROMPT_STR));
}
      ...
      default:
        if ((line->count + 1) < line->buf_len)
        {
          ...
          if (line->cursor < line->count)
          {
            ...
            cliPrintf_P(PSTR("\x1B[4h%c\x1B[4l"), rx_data);
          }
        }
        break;
    }
...
void cliPrintf_P(const char *fmt, ...)
{
  va_list arg;
  va_start (arg, fmt);
  int32_t len;
  ...
  len = vsnprintf_P(p_cli->print_buffer, 256, fmt, arg);
  va_end (arg);
...
float cliArgsGetFloat(uint8_t index)
{
  ...
  ret = (float)strtod((const char * ) p_cli->cmd_args.argv[index], (char **)NULL);
...
void cliMemoryDump(cli_args_t *args)
{
  ...
  addr   = (unsigned int *)atoi((const char * ) argv[0]);
  ...
  for (idx = 0; idx<size; idx++)
  {
    if((idx%4) == 0)
    {
      cliPrintf_P(PSTR(" 0x%08X: "), (unsigned int)addr);
    }
    cliPrintf_P(PSTR(" 0x%08X"), *(addr));
...
void cliMemoryDump_P(cli_args_t *args)
{
  ...
  if(args->argc < 1)
  {
    cliPrintf_P(PSTR(">> mdp addr [size] \n"));
    return;
  }
  ...
  addr   = (uint_farptr_t)strtoul((const char * ) argv[0], (char **)NULL, (int) 0);  
  ascptr = (uint_farptr_t)addr;
  ...
  for (idx = 0; idx<size; idx++)
  {
    ...
    cliPrintf_P(PSTR(" 0x%08X"), pgm_read_dword_far(addr));
