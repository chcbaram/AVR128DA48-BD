...
bool lcdInit(void)
{
  ...
  frame_buffer[0] = ssd1306GetBuffer();
  p_draw_frame_buf = ssd1306GetBuffer();
#endif
...
LCD_OPT_DEF void lcdDrawPixel(uint16_t x_pos, uint16_t y_pos, uint32_t rgb_code)
{
  ...
  if (rgb_code > 0)
  {
    p_draw_frame_buf[x_pos + (y_pos / 8) * LCD_WIDTH] |= 1 << (y_pos % 8);
  }
  else
  {
    p_draw_frame_buf[x_pos + (y_pos / 8) * LCD_WIDTH] &= ~(1 << (y_pos % 8));
...
void lcdPrintf_P(int x, int y, uint16_t color,  const char *fmt, ...)
{
  ...
  len = vsnprintf_P(print_buffer, 255, fmt, arg);
  va_end (arg);
...
void disEngFont(int x, int y, char ch, lcd_font_t *font, uint16_t textcolor)
{
  ...
  {    
    b = pgm_read_word_near(&font->data[(ch - 32) * font->height + i]);
...
void cliLcd(cli_args_t *args)
{
  ...
  if (args->argc == 1 && args->isStr(0, "test") == true)
  {
    ...
    while(cliKeepLoop())
    {
      if (lcdDrawAvailable() == true)
      {
        ...
        lcdPrintf_P(0,16*0, green, PSTR("[LCD 테스트]"));

        lcdPrintf_P(0,16*1, white, PSTR("%d fps"), lcdGetFps());
  ...
  if (ret != true)
  {
    cliPrintf_P(PSTR("lcd test\n"));
    cliPrintf_P(PSTR("lcd bl 0~100\n"));
  }
}
#endif
