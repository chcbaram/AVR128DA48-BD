...
uint16_t dxlUpdateCrc(uint16_t crc_accum, uint8_t *data_blk_ptr, uint16_t data_blk_size)
{
    ...
    for(j = 0; j < data_blk_size; j++)
    {
      ...
      crc_data = pgm_read_word_far(&crc_table[i]);
      crc_accum = (crc_accum << 8) ^ crc_data;
    }
...
static void cliDxl(cli_args_t *args)
{
  ...
  if (args->argc == 2 && args->isStr(0, "open"))
  {
    ...
    cliPrintf_P(PSTR("dxlOpen ch%d %ld bps\n"), _DEF_DXL1, baud);
    ret = true;
  }
  ...
  if (args->isStr(0, "ping"))
  {
    ...
    {
      cliPrintf_P(PSTR("dxl port not open\n"));
      return;
    }
  ...
  if (args->argc == 4 && args->isStr(0, "read"))
  {
    ...
    if (dxl_ret == true)
    {
      ...
      {
        cliPrintf_P(PSTR("rx %d:0x%02X (%d)\n"),
                  dxl_addr + i,
    ...
    {
      cliPrintf_P(PSTR("dxlInstRead Fail : 0x%X\n"), cli_dxl.packet.err);
    }
  ...
  if (args->argc == 5 && args->isStr(0, "write"))
  {
    ...
    if (dxl_ret == true)
    {
      cliPrintf_P(PSTR("dxlInstWrite OK\n"));
    }
    else
    {
      cliPrintf_P(PSTR("dxlInstWrite Fail : 0x%X\n"), cli_dxl.packet.err);
    }
  ...
  if (args->argc == 2 && args->isStr(0, "test_led"))
  {
    ...
    while(cliKeepLoop())
    {
      ...
      if (dxl_ret == false)
      {
        cliPrintf_P(PSTR("dxlInstWrite Fail\n"));
        break;
      }
      delay(500);
    }
  ...
  if (args->argc >= 4 && args->isStr(0, "sync_read"))
  {
    ...
    if (dxl_ret == true)
    {
      ...
      for (int i=0; i<sync_read.resp.id_cnt; i++)
      {
        ...
        {
          cliPrintf_P(PSTR("   data[%d] : 0x%X(%d)\n"),
                    j,
    ...
    {
      cliPrintf_P(PSTR("dxlInstSyncRead Fail : 0x%X\n"), cli_dxl.packet.err);
    }
  ...
  if (args->argc >= 5 && args->isStr(0, "sync_write"))
  {
    ...
    {
      cliPrintf_P(PSTR("dxlInstSyncWrite Fail \n"));
    }
  ...
  if (args->argc >= 4 && args->isStr(0, "bulk_read"))
  {
    ...
    {
      cliPrintf_P(PSTR("dxlInstBulkRead Fail : 0x%X\n"), cli_dxl.packet.err);
    }
  ...
  if (args->argc >= 5 && args->isStr(0, "bulk_write"))
  {
    ...
    {
      cliPrintf_P(PSTR("dxlInstBulkWrite Fail \n"));
    }
  ...
  if (args->argc == 3 && args->isStr(0, "test_motor"))
  {
    ...
    while(1)
    {
      if (millis()-pre_time >= 100)
      {
        ...
        dxl_inst.sync_read.param.id_cnt = 2;
        dxl_inst.sync_read.param.addr   = 132; // Present Position
        dxl_inst.sync_read.param.length = 4;
        dxl_inst.sync_read.param.id[0]  = id_l;
        dxl_inst.sync_read.param.id[1]  = id_r;


        if (dxlInstSyncRead(&cli_dxl, &dxl_inst.sync_read, 100) == true)
        {
          for (int i=0; i<dxl_inst.sync_read.resp.id_cnt; i++)
          {
            cliPrintf_P(PSTR("ID : %d, "), dxl_inst.sync_read.resp.node[i].id);

            uint32_t rx_data = 0;

            rx_data  = (uint32_t)dxl_inst.sync_read.resp.node[i].data[0] << 0;
            rx_data |= (uint32_t)dxl_inst.sync_read.resp.node[i].data[1] << 8;
            rx_data |= (uint32_t)dxl_inst.sync_read.resp.node[i].data[2] << 16;
            rx_data |= (uint32_t)dxl_inst.sync_read.resp.node[i].data[3] << 24;

            cliPrintf_P(PSTR("%ld "), rx_data);
          }
          cliPrintf_P(PSTR("\n"));
        }
      }
  ...
  if (ret == false)
  {
    cliPrintf_P(PSTR("dxl open baud\n"));
    cliPrintf_P(PSTR("dxl ping id\n"));
    cliPrintf_P(PSTR("dxl read id addr len\n"));
    cliPrintf_P(PSTR("dxl write id addr data len(~4)\n"));
    cliPrintf_P(PSTR("dxl sync_read addr len id1 id2 ...\n"));
    cliPrintf_P(PSTR("dxl sync_write addr len data id1 id2 ...\n"));

    cliPrintf_P(PSTR("dxl bulk_read  [addr len id1] [addr len id1] ...\n"));
    cliPrintf_P(PSTR("dxl bulk_write [addr len data id1] [addr len data id2] ...\n"));


    cliPrintf_P(PSTR("dxl test_led id\n"));
    cliPrintf_P(PSTR("dxl test_motor id_l id_r\n"));
  }
}
#endif