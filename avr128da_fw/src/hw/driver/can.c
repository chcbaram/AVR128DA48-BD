...
bool canInit(void)
{
  ...
  for(i = 0; i < CAN_MAX_CH; i++)
  {
    ...
    qblockCreateBySize(&can_tbl[i].q_msg, (uint8_t *)&can_tbl[i].can_msg[0], sizeof(can_msg_t), CAN_MSG_RX_BUF_MAX);
  }
...
uint32_t canMsgAvailable(uint8_t ch)
{
  ...
  return qblockAvailable(&can_tbl[ch].q_msg);
}
...
bool canMsgRead(uint8_t ch, can_msg_t *p_msg)
{
  ...
  ret = qblockRead(&can_tbl[ch].q_msg, (uint8_t *)p_msg, 1);
...
void canRxFifoUpdate(uint8_t ch)
{
  ...
  rx_buf  = (can_msg_t *)qblockPeekWrite(&can_tbl[ch].q_msg);
  ...
  if (qblockWrite(&can_tbl[ch].q_msg, NULL, 1) != true)
  {
  ...
  if( can_tbl[ch].handler != NULL )
  {
    ...
    {
      qblockRead(&can_tbl[ch].q_msg, NULL, 1);
    }
  }
}
